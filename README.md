...
Stable diffusion, imagen, midjourney work using and encoder netowrk like frozen CLIP and classifier free guidance, in the latent space: UNet with a scheduler to draw by detecting edges and curves in a preset or randomly generated "seed" gaussian noise image and expands out of latent space for a variational autoencoder decoder that cleans the images of any remaining noise 
...
Additionally two open source demo models [CogVideo](https://github.com/THUDM/CogVideo) by a groups of cs students and a model by [Antonia Antonova](https://antonia.space/text-to-video-generation) have presented their own innovative methods of generating video from text. There is also Facebook's [Make-A-Video](https://makeavideo.studio/)
